...
int main(int argc, char const *argv[]) {
    ...
    std::string dirLeft = std::string("./in/left");
    std::string dirRight = std::string("./in/right");
    ...
    {
        std::cout << filenamesLeft[i] << std::endl;
    }
    ...
    {
        cv::Mat imageLeft = cv::imread("./in/left/"+filenamesLeft[i]);
        cv::Mat imageRight = cv::imread("./in/right/"+filenamesRight[i]);
    ...
    double stereoRMS = cv::stereoCalibrate(objectPoints,imagePointsLeft,imagePointsRight,intrinsicLeft,distCoeffsLeft,intrinsicRight,distCoeffsRight,imagesLeft[0].size(),R,T,E,F,cv::TermCriteria(CV_TERMCRIT_ITER | CV_TERMCRIT_EPS, 300, 1e-7));
    ...
    if(DISP == 1) {
      disparity(undistortedLeft[0], undistortedRight[0], disparityMap);
      disparityMap*=(1/16.0);
